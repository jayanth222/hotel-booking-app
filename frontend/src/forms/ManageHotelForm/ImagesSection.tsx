...
const ImageSection =()=>{
    const {register,formState: {errors} ,setValue, watch}=useFormContext<HotelFormData>();
                    ...
                    <div className="grid grid-cols-6 gap-4">
                        {existingUrls.map((url)=>(
                            <div className="relative group">
                                ...
                                className="absolute inset-0  flex items-center justify-center text-white bg-black bg-opacity-50 opacity-0 group-hover:opacity-100">Delete</button>
                            </div>
                        ))}
                    </div>
                )}
